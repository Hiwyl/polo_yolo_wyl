...
def get_random_data(annotation_line, input_shape, random=True, max_boxes=80, jitter=.1, hue=.1, sat=1.5, val=1.5, proc_img=True):
    ...
    image = Image.open(line[0]) #usage of opencv will speed up it massively
    iw, ih = image.size
    h, w = input_shape
    #splits the input text labels to boxes and their coordinates
    ...
    if new_ar < 1:
        ...
        nw = int(scale * w)
        nh = int(nw / new_ar)
    image = image.resize((nw, nh), Image.BICUBIC) #rewrite to opencv for speed-up
...
if __name__ == "__main__":
    ...
    def _main():
        ...
        annotation_path = './dataset/wyl_train.txt'
        validation_path = './dataset/wyl_val.txt'
        log_dir = 'weights/'
        ...
        input_shape = (32,32) # multiple of 32, hw  416,832
        ...
        model.save(log_dir+"/polo_yolo.h5")
